...
function loadFolderContents(folderId, subfolderPath = null) {
    ...
    let url = `/engine/get_folder_contents/?folder_id=${encodeURIComponent(folderId)}`;
    if (subfolderPath) {
        url = `/engine/get_folder_contents/?subfolder_path=${encodeURIComponent(subfolderPath)}`;
    }
...
function openFile(file) {
    ...
    if (file.is_dir) {
        ...
        const fileUrl = `/engine/file_action/?action=open&path=${encodeURIComponent(file.id)}`;
        window.open(fileUrl, '_blank');
    } else {
        fetch(`/engine/file_action/?action=open&path=${encodeURIComponent(file.id)}`)
            .then(response => response.json())
            .then(data => {
