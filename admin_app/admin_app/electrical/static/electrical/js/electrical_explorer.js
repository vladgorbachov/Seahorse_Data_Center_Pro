...
function loadFolderContents(folderId, subfolderPath = null) {
    ...
    let url = `/electrical/get_folder_contents/?folder_id=${encodeURIComponent(folderId)}`;
    if (subfolderPath) {
        url = `/electrical/get_folder_contents/?subfolder_path=${encodeURIComponent(subfolderPath)}`;
    }
...
function openFile(file) {
    ...
    if (file.is_dir) {
        ...
        const fileUrl = `/electrical/file_action/?action=open&path=${encodeURIComponent(file.id)}`;
        window.open(fileUrl, '_blank');
    } else {
        fetch(`/electrical/file_action/?action=open&path=${encodeURIComponent(file.id)}`)
            .then(response => response.json())
            .then(data => {
...
function showFileInfo(file) {
    fetch(`get_file_info/?path=${encodeURIComponent(file.id)}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
