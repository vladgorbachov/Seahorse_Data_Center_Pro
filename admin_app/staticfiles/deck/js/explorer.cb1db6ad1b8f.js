...
function displayFiles(files) {
    ...
    files.forEach(file => {
        ...
        fileItem.innerHTML = `
            ...
            <div class="file-type">${file.file_type || 'Unknown'}</div>
        `;
...
function handleFileClick(file) {
    ...
    fileList.querySelector(`[data-file-id="${file.id}"]`).classList.add('selected');
}
...
function handleContextMenu(e, file) {
    ...
    openFileLink.onclick = () => openFile(file);
    renameFileLink.onclick = () => renameFile(file);
    deleteFileLink.onclick = () => deleteFile(file);
}
