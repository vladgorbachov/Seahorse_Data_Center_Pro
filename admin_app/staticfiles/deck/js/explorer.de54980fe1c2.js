// Определяем функцию loadFolderContents в глобальной области видимости
    ...
    fetch(`/deck/get_folder_contents/?folder_id=${folderId}`)
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                updateBreadcrumb(data.current_path, folderId);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Инициализация
    const folderId = document.getElementById('currentPath').dataset.folderId;
    if (folderId) {
        loadFolderContents(folderId);
    }
});