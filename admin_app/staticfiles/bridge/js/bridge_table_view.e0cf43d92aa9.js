document.addEventListener('DOMContentLoaded', () => {
    ...
    saveTableDimensionsButton.addEventListener('click', () => {
        ...
        if (rows && columns) {
            createBridgeTable(rows, columns);
        }
    });
    ...
    function createBridgeTable(rows, columns) {
        fetchWithTimeout('/bridge/bridge_save_table/', {
            method: 'POST',
            headers: {
    ...
    saveTableButton.addEventListener('click', () => {
        ...
        fetchWithTimeout('/bridge/bridge_save_table_data/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
    ...
    deleteTableButton.addEventListener('click', () => {
        ...
        if (confirmDelete) {
            fetchWithTimeout(`/bridge/bridge_delete_table_in_folder/${folderId}/`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': getCookie('csrftoken')
