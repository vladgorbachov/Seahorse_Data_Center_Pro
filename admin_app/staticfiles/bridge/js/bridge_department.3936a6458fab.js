document.addEventListener('DOMContentLoaded', () => {
    ...
    function addFolderEventHandlers(folder) {
        folder.addEventListener('click', () => {
            ...
            if (url && url.startsWith('file://')) {
                window.location.href = `/bridge/explorer/${folderId}/`;  // Обновлен путь
            } else {
                window.location.href = `/bridge/folder/${folderId}/`;  // Обновлен путь
            }
        });
    ...
    function getMaxFolderIndex() {
        return fetch('/bridge/get_max_folder_index/')  // Обновлен путь
    ...
    function saveFolderToServer(index, name) {
        return fetch('/bridge/add_folder/', {  // Обновлен путь
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
            },
            body: JSON.stringify({ index: index, name: name, department: 'bridge' })  // Добавлен параметр department
        })
        .then(response => response.json())
        .then(data => {
    ...
    function updateFolderOnServer(id, updates) {
        return fetch(`/bridge/update_folder/${id}/`, {  // Обновлен путь
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
    ...
    function deleteFolderFromServer(id) {
        ...
        fetch(`/bridge/delete_folder/${id}/`, {  // Обновлен путь
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
