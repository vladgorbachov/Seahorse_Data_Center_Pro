...
function loadFolderContents(folderId, subfolderPath = null) {
    ...
    let url = `/bridge/bridge_get_bridge_folder_contents/?folder_id=${encodeURIComponent(folderId)}`;
    if (subfolderPath) {
        url = `/bridge/bridge_get_bridge_folder_contents/?subfolder_path=${encodeURIComponent(subfolderPath)}`;
    }
...
function renameFile(file) {
    ...
    if (newName && newName !== file.name) {
        fetch('/bridge/bridge_rename_bridge_file/', {
            method: 'POST',
            headers: {
...
function deleteFile(file) {
    if (confirm(`Are you sure you want to delete ${file.name}?`)) {
        fetch('/bridge/bridge_delete_bridge_file/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
...
function showFileInfo(file) {
    fetch(`/bridge/bridge_get_bridge_file_info/?path=${encodeURIComponent(file.id)}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
