document.addEventListener('DOMContentLoaded', () => {
    ...
    inputSaveButton.addEventListener('click', () => {
        ...
        if (folderName) {
            getMaxBridgeFolderIndex().then(maxIndex => {
                const newIndex = maxIndex + 1;
                saveBridgeFolderToServer(newIndex, folderName).then(data => {
                    ...
                    addBridgeFolderEventHandlers(folder);
    ...
    function addBridgeFolderEventHandlers(folder) {
        folder.addEventListener('click', () => {
            const folderId = folder.dataset.id;
            const url = folder.dataset.url;
            console.log('Folder URL:', url);
            if (url && url.startsWith('file://')) {
                window.location.href = `/bridge/explorer/${folderId}/`;
            } else {
                window.location.href = `/bridge/folder/${folderId}/`;
            }
        });
        ...
        folder.addEventListener('contextmenu', (e) => {
            ...
            renameLink.addEventListener('click', () => {
                ...
                if (newName) {
                    ...
                    updateBridgeFolderOnServer(folder.dataset.id, { name: newName });
                }
                document.body.removeChild(menu);
            });
            ...
            deleteLink.addEventListener('click', () => {
                ...
                deleteBridgeFolderFromServer(folder.dataset.id);
                document.body.removeChild(menu);
            });
            ...
            addLinkLink.addEventListener('click', () => {
                ...
                linkOkButton.addEventListener('click', () => {
                    ...
                    if (url) {
                        updateBridgeFolderOnServer(folder.dataset.id, { link: url }).then(data => {
                            folder.dataset.url = data.link;  // Установить значение data-url
    ...
    function getMaxBridgeFolderIndex() {
        return fetch('/bridge/get_max_folder_index/')
    ...
    function saveBridgeFolderToServer(index, name) {
        return fetch('/bridge/add_folder/', {
            method: 'POST',
            headers: {
    ...
    function updateBridgeFolderOnServer(id, updates) {
        return fetch(`/bridge/update_folder/${id}/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
    ...
    function deleteBridgeFolderFromServer(id) {
        if (id === undefined) {
            console.error('Folder ID is undefined');
            return;
        }
        fetch(`/bridge/delete_folder/${id}/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken')
    ...
    document.querySelectorAll('.folder').forEach(addBridgeFolderEventHandlers);
});
